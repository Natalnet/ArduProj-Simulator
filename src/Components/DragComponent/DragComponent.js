...
export default function DragComponentIndex({
    ...
    function createSvgComponent() {
        for (
            ...
            let p =
                breadboardView.querySelectorAll("[layer=breadboard]")[index];
    ...
    function removeComponent() {
        //! NÃO REMOVE DO CONNECTIVITY MTX
        ...
        removedComponent.connectors.forEach((c) => {
            if (c.connectedTo.length > 0) {
                c.connectedTo.forEach((toBeRemoved) => {
            ...
            filteredMap.forEach((f) => {
                if (f.id === splitedL[1]) {
                    f.connectors.forEach((c) => {
                        ...
                        console.log(c);
                        c.connectedTo = c.connectedTo.filter((connected) => {
        ...
        let toRemoveLines = lines.filter((i) => {
            return (
                i.startLine.split("/")[1] === id ||
                i.endLine.split("/")[1] === id
            );
        });
